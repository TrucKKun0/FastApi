...
from schema import PostCreate,PostResponse,UserCreate,PostUpdated,UserUpdate,UserPublic,PaginatedPostResponse
from typing import Annotated
from fastapi import FastAPI, Request , HTTPException,status,Depends,APIRouter,Query
...
async def delete_one_post(post_id : int, current_user : CurrentUser,db: Annotated[AsyncSession, Depends(get_db)]):
...
async def update_post_full(post_id : int,current_user : CurrentUser ,db: Annotated[AsyncSession, Depends(get_db)],post_data : PostCreate):
...
async def update_post_partial(post_id : int, current_user : CurrentUser,db: Annotated[AsyncSession, Depends(get_db)],post_data : PostUpdated):
    result = await db.execute(select(models.Post).where(models.Post.id == post_id))
    post = result.scalars().first()
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail = "Post not found")
